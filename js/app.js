...
var CELULAR_EMPRESA = '5561995757595';
...
cardapio.metodos = {
    ...
    resumoPedido: () => {
        ...
        // let nome = $("#txtNome").val().trim().replace(/\D/g, ''); // trocar para telefone
        let sala = $("#txtSala").val().trim();
        ...
        if (nome != "") {
            ...
            cardapio.metodos.mensagem("Digite seu nome, por favor.");
            $("#txtNome").focus();
        }
        ...
        if (sala.length <= 0) {
            cardapio.metodos.mensagem('Informe a sala, por favor.');
            $("#txtSala").focus();
            return;
        }
    ...
    carregarResumo: () => {
        ...
        $("#resumoEndereco").html(`${MEU_ENDERECO.nome}`);
        $("#cidadeEndereco").html(`${MEU_ENDERECO.sala}`);
